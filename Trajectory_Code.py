...
def m(t):
    if t <= tb1:
        ...
        return mpl
# the L/D Ratio here is TINY. Drag has an alpha squared term
def cL(alpha):
...
# Now switch to wave-glider
    else:
        return 0.5*rho*(vrx**2+vry**2)*0.015*cD(alphaf(t))
...
def L(t,vrx,vry):
    ...
    if t<= tb1 + 75:
        ...
        return 0.5*rho*(vrx**2+vry**2)*0.015*cL(alphaf(t))

# Defining the angle of attack change leading to glide
    
def alphaf(t):
    if t <= tb1 + 75:
        return 0
    elif tb1 + 75 < t <= tb1 + 80:
        return -2*(t - (tb1+75)) 
    elif tb1 + 80 < t <= tgf:
        return -10
    else:
...
#        dvrxdt = (L(t,vrx,vry)*np.sin(np.deg2rad(alphaf(t))) - (D(t,vrx,vry)*np.cos(np.deg2rad(alphaf(t)))))
        dvrydt = (L(t,vrx,vry)*np.cos(np.deg2rad(alphaf(t))) + D(t,vrx,vry)*np.cos(np.deg2rad(alphaf(t))))/(m(t)*g)
...
#Return these variables to use and plot 
        ...
        dtheta[i] = np.rad2deg(np.arctan(vy[i]/vx[i]))
        dalpha[i] = alphaf(t)
        dcL[i] = cL(dalpha[i])
        dcD[i] = cD(dalpha[i])
